...
from source.config import COL_NAME, SHEET_NAME_122, FILE_NAME_CHECK_REPORT, main_folder
...
def comparison_columns_in_data():
    ...
    for file in tqdm(files_source):
        ...
        for sheet in range(len(sheets)):
            ...
            # print(f'{file_name} в листе {sheets[sheet]} -> {column_to_list}\n')
            ...
            # условие, если полученные колонки не совпадают с эталонными,
            # то добавляем в словарь mistake_dict ключ - имя файла, значение - лист
            if dict_of_inc_list_files[file_name][sheets[sheet]] != COL_NAME[sheet]:
                # print(incorrect_dict_of_files)
                incorrect_list_of_sheets.append()
                mistake_dict[file_name] = incorrect_list_of_sheets
            else:
                pass
    print(mistake_dict)
    # df = pd.DataFrame(mistake_dict)
    # try:
